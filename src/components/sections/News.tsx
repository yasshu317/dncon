        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <div className="card p-6">
              <p className="text-xs text-slate-500 mb-5">Free seminars at our premises on IT topics:</p>
              <div className="space-y-3">
                {siteConfig.events.map((event, i) => (
                  <div key={i} className="flex items-start gap-3 text-xs text-slate-400 pb-3 border-b border-white/[0.04] last:border-0 last:pb-0">
                    <span className="w-5 h-5 rounded-full bg-white/[0.05] border border-white/[0.08] text-slate-400 flex items-center justify-center text-[10px] font-bold shrink-0">
                      {i + 1}
                    </span>
                    {event}
                  </div>
